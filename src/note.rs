...
pub fn get_volume_from_state_space(
    nss: &NoteStateSpace,
    instrument_id: usize,
    pitch: usize,
) -> f32 {
...
pub fn cache_instruments() -> CachedInstruments {
    ...
    for instr_idx in 0..INSTRUMENT_COUNT {
        ...
        for pitch in 0..PITCH_COUNT {
            ...
            cached_instruments.waveforms[instr_idx]
                .push(wave::change_pitch(&sample_wf, multiplier as f32));
        }
    }

    for instr_idx in 0..INSTRUMENT_COUNT {
        print!(
            "Calculating spectrums for {}\n",
        ...
        for pitch in 0..PITCH_COUNT {
            ...
            let sample_spectrogram = wave::create_spectrum(
                sample_wf_diff_pitch.to_interleaved_samples(),
                sample_wf_diff_pitch.frame_rate_hz(),
                fft_size,
                hop_size,
                -1,
            );
...
pub fn add_notes_together(notes: &[Note], cache: &CachedInstruments, multiplier: f32) -> Waveform {
    if notes.len() == 0 {
        ...
        return Waveform::new(
            cache.waveforms[0][0].frame_rate_hz(),
            cache.waveforms[0][0].num_channels(),
            vec![0.0; 1],
        );
    }

    let max_len_note = notes
        .iter()
        .max_by_key(|note| {
            cache.waveforms[note.instrument_id][note.pitch]
                .to_interleaved_samples()
                .len()
        })
        .unwrap();

    let max_len = cache.waveforms[max_len_note.instrument_id][max_len_note.pitch]
        .to_interleaved_samples()
        .len();
    ...
    for note in notes {
        let samples_to_add =
            cache.waveforms[note.instrument_id][note.pitch].to_interleaved_samples();
    ...
    assert_eq!(
        cache.waveforms[0][0].num_channels(),
        1,
        "We are expecting everything to be mono for now."
    );
...
pub fn add_notes_together_statespace(
    notes: &NoteStateSpace,
    cache: &CachedInstruments,
    multiplier: f32,
) -> Waveform {
    ...
    for instrid in 0..INSTRUMENT_COUNT {
        for pitch in 0..PITCH_COUNT {
            notes_vec.push(Note {
                ...
                pitch,
                volume: get_volume_from_state_space(notes, instrid, pitch),
            })
        }
    }
...
pub fn add_notes_together_merge_from_stsp(
    notes: &[Note],
    volumes: &[f32],
    cache: &CachedInstruments,
    multiplier: f32,
    ...
    for idx in 0..notes.len() {
        notes_vec.push(Note {
            instrument_id: notes[idx].instrument_id,
